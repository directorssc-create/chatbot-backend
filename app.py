...
def chat():
    ...
    if not user_message:
        return jsonify({"response": "Please type a question!", "suggestions": []})

    matches = find_best_match(user_message)

    if matches:
        top_faq = matches[0][1]
        suggestions = related_questions(top_faq)

        response_text = f"\U0001F4A1 **{top_faq['question']}**\n\n{top_faq['answer']}"
        if suggestions:
            suggestion_lines = "\n".join(f"{i}. {q}" for i, q in enumerate(suggestions, 1))
            response_text += f"\n\n\U0001F4CC Related questions you might ask:\n{suggestion_lines}"
