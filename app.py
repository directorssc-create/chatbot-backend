...
FAQS = [
    ...
    },
    {
        ...
        "category": 'Working Professionals Support'
    },
    {
    ...
    },
    {
        ...
        "category": 'Divyang Student Support'
    },
    {
    ...
    },
    {
        ...
        "category": 'Defence Personnel Support'
    },
    {
    ...
    },
    {
        ...
        "category": 'Jail Inmate / Prison Learner Support'
    },
    {
...
# Common Hindi/English connector words that carry no topic meaning on their own.
# Stripped out before comparing word overlap so a query isn't scored just for
# containing "hai" or "kaise" -- only the actual topic words count.
STOPWORDS = {
    "hai", "hain", "ka", "ki", "ke", "ko", "se", "mein", "me", "kya", "kaise",
    "kar", "kare", "karein", "karu", "karna", "karne", "sir", "please",
    "the", "is", "are", "a", "an", "of", "to", "for", "and", "in", "on", "how",
    "what", "do", "does", "can", "i", "my", "please"
}


def tokenize(text):
    """Split text into lowercase alphabetic words, dropping stopwords/1-letter words."""
    words = re.findall(r"[a-zA-Z]+", text.lower())
    return [w for w in words if w not in STOPWORDS and len(w) > 1]


def score_faq(user_message, user_words, faq):
    """
    Score how well one FAQ matches the user's message. Combines three signals so
    that both short queries ("admission") and full sentences ("IGNOU me naya
    admission kaise le?") can match, regardless of word order:
      1. The full keyword phrase appears in the message (strong, e.g. keyword
         "assignment last date" found inside a longer sentence).
      2. The message itself appears inside a longer keyword phrase (handles short
         queries like "admission" matching the keyword "new admission").
      3. Individual topic words overlap between the message and the keyword,
         even if they're not adjacent or in the same order.
    """
    score = 0
    all_kw_words = set()
    for kw in faq["keywords"]:
        if kw in user_message:
            score += 5
        elif len(user_message) >= 4 and user_message in kw:
            score += 4
        all_kw_words |= set(tokenize(kw))

    # Union words across all of this FAQ's keyword phrases before scoring overlap,
    # so a generic word repeated across several phrases (e.g. "assignment" showing
    # up in three separate keyword phrases) can't outscore a FAQ that shares fewer
    # but more specific words with the query.
    score += len(all_kw_words & user_words)
    return score


def find_best_match(user_message):
    user_words = set(tokenize(user_message))
    scored = [(score_faq(user_message, user_words, faq), faq) for faq in FAQS]
    scored = [pair for pair in scored if pair[0] > 0]
    scored.sort(key=lambda pair: pair[0], reverse=True)
    return scored


def related_questions(top_faq, count=4):
    """Pick a handful of other questions from the same category to suggest next."""
    same_category = [
        f for f in FAQS
        if f["category"] == top_faq["category"] and f["question"] != top_faq["question"]
    ]
    random.shuffle(same_category)
    return [f["question"] for f in same_category[:count]]


def faq_by_question(question_text):
    """Look up an FAQ by its exact question text (used when a suggestion chip is clicked)."""
    question_text = question_text.strip()
    for f in FAQS:
        if f["question"].strip().lower() == question_text.lower():
            return f
    return None


def build_payload(faq):
    """Shape one FAQ + its related questions into the JSON the frontend renders."""
    suggestions = related_questions(faq)
    return {
        "question": faq["question"],
        "answer": faq["answer"],
        "category": faq["category"],
        # Each suggestion is its own object so the frontend can render it as a
        # clickable chip/link and send it straight back as the next message.
        "suggestions": [{"text": q} for q in suggestions],
    }


@app.route("/")
def home():
    # Serves the chat UI (templates/index.html). The Flask backend for
    # Google Sites embedding still works exactly the same via /chat.
...
def chat():
    ...
    if not user_message:
        return jsonify({"response": "Please type a question!", "question": None,
                         "answer": None, "suggestions": []})

    # If the incoming message exactly matches one of our known questions (i.e. the
    # user clicked a "related question" chip), answer that FAQ directly instead of
    # re-running the fuzzy matcher -- this keeps clicks 100% accurate.
    exact = faq_by_question(user_message)
    if exact:
        payload = build_payload(exact)
        payload["response"] = f"\U0001F4A1 {payload['question']}\n\n{payload['answer']}"
        return jsonify(payload)

    matches = find_best_match(user_message)

    if matches:
        top_faq = matches[0][1]
        payload = build_payload(top_faq)
        payload["response"] = f"\U0001F4A1 {payload['question']}\n\n{payload['answer']}"
        return jsonify(payload)

    return jsonify({
        "response": "I couldn't find a matching answer. Please try asking about admission, "
                     "assignments, examinations, results, certificates, study material, "
                     "grievances (iGRAM), or scholarships \u2014 or rephrase your question using "
                     "different keywords.",
        "question": None,
        "answer": None,
        "suggestions": []
